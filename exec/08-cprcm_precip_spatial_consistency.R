  ...
  FUN = function(i) {
    ...
    cprcm_data = lapply(
      ...
      FUN = function(id) {
        ...
        out[date %in% obs_dates, .(date, cprcm_precip = precipitation, id = id)]
      })
    ...
    cprcm_stats = obs[, .(
      mean = mean(cprcm_precip, na.rm = TRUE),
      median = median(cprcm_precip, na.rm = TRUE),
      sd = sd(cprcm_precip, na.rm = TRUE),
      min = min(cprcm_precip, na.rm = TRUE),
      max = max(cprcm_precip, na.rm = TRUE),
      n_stations = sum(!is.na(precip))
    ), by = "date"][order(date)]
    ...
    cprcm_stats = cprcm_stats[n_stations >= n_neighbour_min]
