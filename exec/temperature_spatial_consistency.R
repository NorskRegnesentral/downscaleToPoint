...
parallel::mclapply(
  ...
  FUN = function(i) {
    ...
    obs = obs[n_obs_per_date == length(neighbouring_ids)]

    stop("Now we need to add all the covariates that we do inside the for loop. Then, we don't need to load the data again inside the for loop. We can simply filter obs instead!")

    # Simulate temperature at all of the neighbouring IDs
    sims = obs = list()
    for (j in seq_along(neighbouring_ids)) {
      # Compute distances to all other weather stations
      dists = geosphere::distHaversine(
        p1 = station_meta[id == neighbouring_ids[j], c(lon, lat)],
        p2 = station_meta[, cbind(lon, lat)]
      )
      ...
      data = load_station_data(
        meta = station_meta[id == neighbouring_ids[j], ],
        data_dir = data_dir
      )
      data[, let(
        yday = yday(date),
      ...
      obs[[neighbouring_ids[j]]] = data[, .(id, date, tmean)]
      ...
      bad_donors = unique(c(
        get_bad_donor_index(sim, mean, K),
        get_bad_donor_index(sim, sd, K)
      ))
