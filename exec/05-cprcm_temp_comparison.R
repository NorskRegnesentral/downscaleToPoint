...
plot = bootstrap_data |>
  ...
  scale_y_continuous(breaks = seq(-10, 1, by = .2), limits = c(-.2, 1), expand = c(0, 0)) +
  theme_light() +
  theme(
...
x_vars = c("elev", "elev_diff", "dist_to_sea", "tmean_mean")
for (x_var in x_vars) {
...
score_data[, let(scores = lapply(seq_len(.N), function(i) c(full[i], cprcm[i])))]
